...
def init_distributed_mode_ds(args):
    if deepspeed is None:
        print('Deepspeed not available, falling back to regular init')
        return init_distributed_mode(args)
...
def get_train_ds_config(offload,
    ...
    if enable_mixed_precision_lora and dist is not None and get_accelerator is not None:
...
def init_deepspeed(args, model, optimizer, lr_scheduler):
    if deepspeed is None:
        raise RuntimeError("Deepspeed not available but required for training")
